...
class S3FifoCache {
 private:
  ...
  obj_id_t evict_small() {
    while (!small_.empty() && small_bytes_ > small_capacity_) {
      ...
      if (freqs_[idx] > 0) {
        ...
        sizes_[idx] = 0;
        ghost_.push_back(idx);
        ghost_bytes_ += sizes_[idx];
        trim_ghost();
        return victim;
      }
    }
  ...
  void trim_ghost() {
    while (ghost_bytes_ > ghost_capacity_ && !ghost_.empty()) {
      uint32_t idx = ghost_.front();
      ghost_.pop_front();
      if (queues_[idx] == Queue::GHOST) {
        free_idx(idx);
      }
    }
  }
